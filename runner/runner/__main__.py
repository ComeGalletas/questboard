...
--dry-run uses an in-memory DB with the default config; the Supabase connection is wired in a
later PR. Output lists job decisions only, never data.
"""
...
from runner.providers import ClaudeCliProvider
from runner.repo import MemoryRepo, Repo
...
def build(dry_run: bool) -> Scheduler:
    if not dry_run:
        sys.exit("Only --dry-run is available until the Supabase connection lands.")
    repo: Repo = MemoryRepo(Config.model_validate(DEFAULT_CONFIG))
    cli = ClaudeCliProvider()
    return Scheduler(repo=repo, handlers=HANDLERS, providers={cli.name: cli})
